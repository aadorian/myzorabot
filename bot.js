...
const onReceiveMessage = async (msg) => {
    ...
    if (messageContent.startsWith("!minted")) {
        ...
        const mintedEmbed = new discord_js_1.MessageEmbed()
            .setColor(EMBED_COLOR_C)
            .setTitle("Minted")
            .setDescription(resource.mints.__typename + " # " + resource.mints.nodes.length)
    ...
    if (messageContent.startsWith("!zora")) {
        const helpEmbed = new discord_js_1.MessageEmbed()
            .setColor(EMBED_COLOR_C)
            .setTitle("Token")
            .setDescription(response.token.token.description)
